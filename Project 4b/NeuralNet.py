...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        totalModification = 0
        """YOUR CODE"""
        mods=[(alpha*a*delta) for a in [1]+inActs]
        self.weights=[w+m for w,m in zip(self.weights,mods)]
        return sum([abs(m) for m in mods])
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            current=[perp.sigmoidActivation(current) for perp in layer]
            ret.append(current)
        return ret
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            allLayerOutput = self.feedForward(input)
            ...
            for outputNum in xrange(len(example[1])):
                ...
                error = output[outputNum]-lastLayerOutput[outputNum]
                delta = gPrime*error
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    nextDeltas=deltas[0]
                    delta = gPrime*sum([nextLayer[x].weights[neuronNum+1]*nextDeltas[x] for x in range(len(nextLayer))])
            ...
            for numLayer in xrange(0,self.numLayers):
                ...
                deltaL=deltas[numLayer]
                for numNeuron in xrange(len(layer)):
                    weightMod = layer[numNeuron].updateWeights(allLayerOutput[numLayer],alpha,deltaL[numNeuron])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while iteration<maxItr and weightMod>weightChangeThreshold:
        trainError,weightMod=nnet.backPropLearning(training,alpha)
    ...
    testCorrect = 0     
    for e in test:
        output=[round(r) for r in nnet.feedForward(e[0])[-1]]
        if e[1]==output:
            testCorrect=testCorrect+1
        else:
            testError=testError+1
    testAccuracy=testCorrect/float(testError+testCorrect)#num correct/num total
